...
export class usuarioMock {
    ...
    private static usuarioDB: Usuario[] = [
        new Usuario(1, "Renato Fraga", "0000000", ""),
        new Usuario(2, "Jose", "0000000", ""),
        new Usuario(3, "João", "0000000", ""),
        new Usuario(4, "Mario", "0000000", "")
    ];
    ...
    /*static async salvar(usuario: Usuario): Promise<void> {
        ...
        if (indexExistente === -1) {
            ...
            const maiorCodigo = this.usuarioDB.length > 0 
                ? Math.max(...this.usuarioDB.map(u => u.id)) 
                : 0;
            
            usuario.id = maiorCodigo + 1;
            this.usuarioDB.push(usuario);
            console.log(`Usuário de ID ${usuario.id} salvo com sucesso!`);
        } else {
